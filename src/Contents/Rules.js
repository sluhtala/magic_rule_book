...
const Rule = ({rule, activeIndex, id, setActiveIndex, rBook, modalRules, setModalRules})=>{
	const len = 30;
  ...
  const handleLinkClick = (word) => {
	  ...
	  if (dash === -1)
	  {
		  ...
		  setModalRules(subRules.filter((r)=>(r.text.substring(0, r.text.indexOf(' ')).includes(word))))
	  }
	  else{
		  if (isNaN(word[dash - 1]))
		  {
				const len = word[dash + 1].charCodeAt(0) - word[dash - 1].charCodeAt(0) + 1;
				const index = subRules.map((r)=>r.text.substring(0, r.text.indexOf(' '))).indexOf(word.substr(0, dash))
